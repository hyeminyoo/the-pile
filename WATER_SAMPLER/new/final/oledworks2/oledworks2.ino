// THIS IS VERSION2 OLED
// version1 -> version2
// trying to make another display screen after it takes in
// hours and minutes and clicks C
#include <SPI.h>
#include <Wire.h>
...
void loop() {
  ...
  if (buttonStateC != lastButtonStateC) {
    if (buttonStateC == LOW) {
      ...
      display.print(firstA);
      if (firstA == 1 || firstA == 0) {
        display.print(" hr ");
      }
      else
        display.print(" hrs ");
      display.println(firstB);
      if (firstB == 1 || firstB == 0) {
        display.print(" min");
      }
      else
